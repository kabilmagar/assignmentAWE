<x-inputs.front>
    <div class="relative flex justify-center items-center md:flex md:justify-start">
        <img class=" hidden md:block w-full" src="https://images.pexels.com/photos/4522998/pexels-photo-4522998.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="santa">
        <img class=" md:hidden w-full " src="https://images.unsplash.com/photo-1600080972464-8e5f35f63d08?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774&q=80" alt="santa">
        <div class="flex absolute justify-start flex-col md:flex-row items-center md:py-10 lg:py-20 xl:py-40 md:space-y-12 ">
                ...
                <div class="group flex justify-start xl:items-start items-center flex-col">
                    <div class="h-full relative">
                        ...
                        <div class="absolute top-0 right-0 w-12 flex justify-start item-start h-12 tranform duration-500 group-hover:bg-white dark:hover:bg-gray-900 dark:bg-transparent">
                            <p class="ml-2 -mt-2 text-3xl leading-loose group-hover:text-gray-800 text-white tranform duration-500 dark:hover:text-white dark:text-gray-900">01</p>
                        </div>
                    </div>
                </div>
                <div class="group flex justify-start xl:items-start items-center flex-col">
                    <div class="h-full relative">
                        ...
                        <div class="absolute top-0 right-0 w-12 flex justify-start item-start h-12 group-hover:bg-white tranform duration-500 dark:hover:bg-gray-900 dark:bg-transparent">
                            <p class="ml-2 -mt-2 text-3xl leading-loose group-hover:text-gray-800 text-white tranform duration-500 dark:hover:text-white dark:text-gray-900">02</p>
                        </div>
                    </div>
                </div>
                <div class="group flex justify-start xl:items-start items-center flex-col">
                    <div class="h-full relative">
                        ...
                        <div class="absolute top-0 right-0 w-12 flex justify-start item-start h-12 group-hover:bg-white tranform duration-500 dark:hover:bg-gray-900 dark:bg-transparent">
                            <p class="ml-2 -mt-2 text-3xl leading-loose group-hover:text-gray-800 text-white tranform duration-500 dark:hover:text-white dark:text-gray-900">03</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="container xl:w-full w-11/12 mx-auto pt-16">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 pt-20 gap-10 ">
          @foreach($books as $book)
            <div>
                <div class="w-full rounded h-56">
                    <img src="{{$book->url}}" alt="Display picture of Silene Tokyo" role="img" class="object-cover h-full w-full overflow-hidden rounded shadow" />
                </div>
                <div class="flex w-full items-center justify-between pt-6 pb-1">
                    <p class="text-xl font-normal text-gray-800">{{$book->author}}</p>
                </div>
                <p class="text-base text-gray-600 pb-3">{{$book->title}}</p>
                <p class="text-sm text-gray-600 leading-6 font-normal">{{$book->description}}</p>
            </div>
          @endforeach  
      </div>
  </div>
            ...
            <div class="overflow-hidden transition-shadow duration-300 bg-white rounded shadow-sm">
              ...
              <div class="p-5 border border-t-0">
                <p class="mb-3 text-xs font-semibold tracking-wide uppercase">
                  <a  class="transition-colors duration-200 text-blue-gray-900 hover:text-deep-purple-accent-700" aria-label="Category" title="traveling">{{$game->pegi}}</a>
                  <span class="text-gray-600">— {{$game->title}}</span>
                </p>
                <a href="/" aria-label="Category" title="Visit the East" class="inline-block mb-3 text-2xl font-bold leading-5 transition-colors duration-200 hover:text-deep-purple-accent-700">{{$game->console}}</a>
          ...
          @foreach($cds as $cd)
            <div>
              <div class="relative overflow-hidden transition duration-300 transform rounded shadow-lg lg:hover:-translate-y-2 hover:shadow-2xl">
                <img class="object-cover w-full h-56 md:h-64 xl:h-80" src="{{$cd->url}}" alt="Person" />
    ...
    <div class="relative flex justify-center items-center mb-10">
      ...
      <div class="absolute  px-4 flex justify-center items-center flex-col ">
          ...
          @error('email')
            <p class="mt-5 md:mt-6 text-sm md:text-base font-medium leading-none text-gray-300">
                {{$message}}
            </p>
          @enderror  
      </div>
    ...
    @if(session()->has('message'))
      <div role="alert" class="xl:w-5/12 mx-auto sm:mx-0 sm:w-6/12 md:w-6/12 w-10/12 bg-white dark:bg-gray-800 shadow-lg rounded flex pr-4 absolute left-0 sm:left-auto right-0 sm:top-0 sm:mr-6 mt-16 sm:mt-6 mb-6 sm:mb-0 transition duration-150 ease-in-out" id="notification">
        <div class="px-5 border-r border-gray-300 dark:border-gray-700 flex items-center justify-center text-green-400">
          <img src="https://tuk-cdn.s3.amazonaws.com/can-uploader/single_action_with_sub_text-svg1.svg" alt="success logo"/>
        </div>
        <div class="flex flex-col justify-center pl-4 py-4">
          <h1 class="text-sm text-gray-800 dark:text-gray-100 font-semibold">Action Successful</h1>
